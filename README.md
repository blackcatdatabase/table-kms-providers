...
<div style='font-size:13px;opacity:0.92;'>Lineage heat: 0 outbound / 2 inbound · ✅ No engine drift detected · Index coverage: ready · PII: 1 · Changelog: fresh (18 d)</div>
</div>
...
- Pkg folder: [packages\kms-providers](https://github.com/blackcatacademy/blackcat-database/blob/main/packages\kms-providers)
...
graph LR
  ...
  kms_providers["kms_providers"]:::center
  kms_health_checks["kms_health_checks"]:::inbound -->|FK| kms_providers
  kms_keys["kms_keys"]:::inbound -->|FK| kms_providers
...
- Inbound (relies on this): "kms_health_checks", "kms_keys"
...
## Usage
- Install/upgrade schema: pwsh -NoLogo -NoProfile -File scripts/schema-tools/Migrate-DryRun.ps1 -Package kms-providers -Apply
