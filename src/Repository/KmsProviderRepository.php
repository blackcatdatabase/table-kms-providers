...
use BlackCat\Database\Packages\KmsProviders\Repository\KmsProviderRepositoryInterface;
...
class KmsProviderRepository implements KmsProviderRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\KmsProviders\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'provider', 'location', 'project_tenant', 'is_enabled' ]);
          return count($rows);
        }
