...
namespace BlackCat\Database\Packages\KmsProviders\Repository;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\KmsProviders\Definitions;
use BlackCat\Database\Packages\KmsProviders\Criteria;
use BlackCat\Database\Packages\KmsProviders\ContractRepository;

final class KmsProviderRepository implements ContractRepository {
    ...
    public function insert(array $row): void {
        ...
        $sql = "INSERT INTO kms_providers (".implode(',', $cols).") VALUES (".implode(',', $place).")";
    ...
    public function insertMany(array $rows): void {
        ...
        $sql = "INSERT INTO kms_providers (".implode(',', $cols).") VALUES ".implode(',', $allPlace);
        $this->db->execute($sql, $params);
    }
    ...
    public function upsert(array $row): void {
        ...
        $sqlBase = "INSERT INTO kms_providers (".implode(',', $cols).") VALUES (".implode(',', $place).")";
    ...
    public function updateById(int|string $id, array $row): int {
        ...
        $sql = "UPDATE kms_providers SET ".implode(',', $assign)." WHERE id = :id".$verCond;
    ...
    public function deleteById(int|string $id): int {
        ...
        if ($soft) {
            ...
            return $this->db->execute("UPDATE kms_providers SET $set WHERE id = :id", $params);
        }
        return $this->db->execute("DELETE FROM kms_providers WHERE id = :id", [':id'=>$id]);
    }
    ...
    public function restoreById(int|string $id): int {
        ...
        $set = "$soft = NULL";
        if ($updAt && $updAt !== $soft) { $set .= ", $updAt = CURRENT_TIMESTAMP"; }
        return $this->db->execute("UPDATE kms_providers SET $set WHERE id = :id", [':id'=>$id]);
    }

    // ============ READ / PAGE / LOCK ============
    public function findById(int|string $id): ?array {
        $sql = "SELECT * FROM kms_providers WHERE id = :id";
        $row = $this->db->fetchOne($sql, [':id'=>$id]);
        return $row ?: null;
    }

    public function exists(string $whereSql = '1=1', array $params = []): bool {
        $sql = "SELECT 1 FROM kms_providers WHERE $whereSql LIMIT 1";
        return (bool)$this->db->fetchOne($sql, $params);
    }

    public function count(string $whereSql = '1=1', array $params = []): int {
        $sql = "SELECT COUNT(*) FROM kms_providers WHERE $whereSql";
    ...
    public function paginate(Criteria $c): array {
        ...
        $total = (int)$this->db->fetchOne("SELECT COUNT(*) FROM kms_providers t $joins WHERE $where", $params);
        $items = $this->db->fetchAll("SELECT t.* FROM kms_providers t $joins WHERE $where ORDER BY $order LIMIT $limit OFFSET $offset", $params);
    ...
    public function lockById(int|string $id): ?array {
        $sql = $this->db->isMysql()
            ? "SELECT * FROM kms_providers WHERE id = :id FOR UPDATE"
            : "SELECT * FROM kms_providers WHERE id = :id FOR UPDATE";
