...
use BlackCat\Database\Packages\KmsProviders\Dto\KmsProviderDto as Dto;
use BlackCat\Database\Packages\KmsProviders\Mapper\KmsProviderDtoMapper as RowMapper;
...
final class KmsProviderRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\KmsProviders\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'provider', 'location', 'project_tenant', 'is_enabled' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'name' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\KmsProviders\Definitions::table());
            return $bulk->upsertMany($rows, [ 'name' ], [ 'provider', 'location', 'project_tenant', 'is_enabled' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\KmsProviders\Dto\KmsProviderDto|null */
    public function getByName(string $name, bool $asDto = false): array|\BlackCat\Database\Packages\KmsProviders\Dto\KmsProviderDto|null {
        $row = $this->getByUnique([ 'name' => $name ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\KmsProviders\Mapper\KmsProviderDtoMapper::fromRow($row);
    }
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\KmsProviders\Dto\KmsProviderDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\KmsProviders\Dto\KmsProviderDto|null {
