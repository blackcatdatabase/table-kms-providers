 ...
 * - whitelist LIKE:   [ 'name', 'provider', 'location', 'project_tenant' ]
 * Podporované tvary:
...
final class Criteria {
    ...
    public function toSql(bool $viewMode = false): array {
        ...
        $allowed = array_fill_keys([ 'id', 'name', 'provider', 'location', 'project_tenant', 'created_at', 'is_enabled' ], true);
        ...
        $order = null;
        if ($this->sort) {
            $safeCols = array_fill_keys([ 'id', 'name', 'provider', 'location', 'project_tenant', 'created_at', 'is_enabled' ], true);
            $parts = [];
