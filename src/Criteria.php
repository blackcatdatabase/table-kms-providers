...
final class Criteria extends BaseCriteria
{
    ...
    {
        return [ 'id', 'name', 'provider', 'location', 'project_tenant', 'created_at', 'is_enabled' ];
    }

    /** Columns used for full-text LIKE/ILIKE searches. */
    protected function searchable(): array
    {
        return [ 'name', 'location', 'project_tenant' ];
    }

    /** Columns allowed in ORDER BY (falls back to filterable() when empty). */
    protected function sortable(): array
    {
        $x = [ 'id', 'name', 'provider', 'location', 'project_tenant', 'created_at', 'is_enabled' ];
    ...
    public static function fromDb(
        ...
        if (\method_exists(\BlackCat\Database\Packages\KmsProviders\Definitions::class, 'softDeleteColumn')) {
            $soft = \BlackCat\Database\Packages\KmsProviders\Definitions::softDeleteColumn();
