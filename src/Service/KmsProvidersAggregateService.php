...
namespace BlackCat\Database\Packages\KmsProviders\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\KmsProviders\Dto\KmsProviderDto;
use BlackCat\Database\Packages\KmsProviders\Mapper\KmsProviderDtoMapper;
 ...
 */
final class KmsProvidersAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private KmsProviderRepository $kmsProviderRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'kms_providers' => $this->kmsProviderRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\KmsProviders\ModuleException("Record {$idStr} not found for locking.");
            }
